...
from tensorflow.keras.layers import Dense, Dropout, Flatten, Input, Conv3D
...
def get_int_model(model, layer, backend='tf'):
    ...
    int_model.add(Conv3D(64, kernel_size=(3, 3, 3), activation='relu',
                                padding='same', name='conv1',
    ...
    # 2nd layer group
    int_model.add(Conv3D(128, kernel_size=(3, 3, 3), activation='relu',
                                padding='same', name='conv2',
    ...
    # 3rd layer group
    int_model.add(Conv3D(256, kernel_size=(3, 3, 3), activation='relu',
                                padding='same', name='conv3a',
    ...
    # 4th layer group
    int_model.add(Conv3D(512, kernel_size=(3, 3, 3), activation='relu',
                                padding='same', name='conv4a',
...
if __name__ == '__main__':
    ...
    model_modif = keras.models.load_model("sports1M_weights_tf.h5")
    layer_output = model_modif.get_layer('fc7').output
    output = model_modif(input)
    output = Dense(12)(output)
"""
    input = Input(input_shape)
    model_modif = ...... création du modèle
    couche = model_modif(input)
    couche = Conv3D(...)(couche)
    couche = Flatten()(couche)
    couche = Dense(nb_classes)(couche)
    model = Model(input,couche)
"""
